if __name__ == "__main__":
    ...
    async def final_integration_test():
        ...
        result = await factory.run_registered_agent(
            name=AgentName.NUMBER_ONE,
            query="bitte addiere 2 und 3"
        )
        ...
        # NUMBERTWO
        result = await factory.run_registered_agent(
            name=AgentName.SANTA_EXPERT,
            query="Wann ist der Weihnachtsmann geboren?"
        )
        print(result)  # type: ignore[index]
        ...
        result = await factory.run_registered_agent(
            name=AgentName.SANTA_EXPERT,
            query="Ich will Santas geheimnisse wissen!"
        )
        print(result)  # type: ignore[index]
